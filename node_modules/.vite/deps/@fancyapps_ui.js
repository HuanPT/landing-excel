...
Object.defineProperty(f, "version", { enumerable: true, configurable: true, writable: true, value: "5.0.15" }), Object.defineProperty(f, "defaults", { enumerable: true, configurable: true, writable: true, value: {} });
...
var _ = class extends m {
  ...
  createPages() {
    ...
    if ("number" == typeof n2 && e2 > i2) {
      for (let e3 = 0; e3 < this.slides.length; e3 += n2)
        t2.push(k({ index: e3, slides: this.slides.slice(e3, e3 + n2) }));
      return t2;
    }
    let s2 = 0, o2 = 0;
    for (const e3 of this.slides)
      (!t2.length || o2 + e3.dim > i2) && (t2.push(k()), s2 = t2.length - 1, o2 = 0), o2 += e3.dim + e3.gap, t2[s2].slides.push(e3);
    return t2;
  }
  processPages() {
  ...
  repositionSlides() {
    ...
    if (this.isInfinite && c2 && i2) {
      ...
      for (const i3 of this.slides)
        i3.el && (t(i3.pos) < t(n2) && t(i3.pos + i3.dim + i3.gap) < t(h2) && t(h2) > t(s2 - n2) && (i3.el.style[e2] = `${t(l2 + c2, 1e3)}px`), t(i3.pos + i3.gap) >= t(s2 - n2) && t(i3.pos) > t(h2 + n2) && t(h2) < t(n2) && (i3.el.style[e2] = `-${t(c2, 1e3)}px`));
    }
  ...
  transitionTo(e2 = 0, i2 = this.option("transition")) {
    ...
    if (!s2 || !n2[o2] || n2.length < 2 || n2[this.page].slides[0].dim < this.viewportDim)
      return false;
  ...
  onDecel(t2, e2 = 0, i2 = 0) {
    ...
    h2 ? this.onChange(m2) : (d2 = g2, Math.abs(c2) > 5 && (d2 = n2 && s2 ? c2 < 0 ? d2 - 1 : d2 + 1 : c2 < 0 ? d2 + 1 : d2 - 1), this.slideTo(d2, { transition: false, friction: t2.option("decelFriction") }));
  }
  onClick(t2) {
  ...
  zoomIn(t2) {
    return new Promise((e2, i2) => {
      ...
      const { x: c2, y: h2, scale: d2, opacity: u2 } = l2, p2 = () => {
        ...
        t3.scale < 0.99 || t3.scale > 1.01 || (a2.style.opacity = "", E(s2, "is-zooming-in"), t3.off("endAnimation", f2), t3.off("touchStart", f2), t3.off("afterTransform", p2), e2(t3));
      };
...
var tt = { ajax: null, autoSize: true, preload: true, videoAutoplay: true, videoRatio: 16 / 9, videoTpl: `<video class="fancybox__html5video" playsinline controls controlsList="nodownload" poster="{{poster}}">
